...
ts_constant_orig = \
    pd.read_pickle(os.path.join(res_dir, file_name_constant))
...
for grid_id in grids:
    ...
    try:
        ...
        edisgo_obj.results.to_csv(f"{res_dir}/grid_reinforcement_results/{grid_id}/base")
        ...
        # add new time series
        loads_index = random.sample(
            range(0, len(ts_constant_orig.columns)), nr_residential_loads)
        ...
        for file_name_dynamic in files_name_dynamic:
            ts_dynamic_orig = \
                pd.read_pickle(os.path.join(res_dir, file_name_dynamic),
            ...
            for share_dyn in [0.0, 0.5, 1.0]:
                for mode in ["feed-in", "load", "debug"]:
                    ...
                    edisgo_obj.analyze()
                    res_dir_tmp = \
                        f"{res_dir}/grid_reinforcement_results/{grid_id}/" \
                        f"{file_name_dynamic}/{share_dyn}/{mode}"
                    edisgo_obj.results.to_csv(
                        f"{res_dir_tmp}/results_before_reinforcement")
                    edisgo_obj.reinforce(timesteps_pfa="reduced_analysis")
                    edisgo_obj.analyze()
                    edisgo_obj.results.to_csv(
                        f"{res_dir_tmp}/results_after_reinforcement")
    except Exception:
        print(f"Something went wrong with grid {grid_id}.")
        traceback.print_exc()
print("Success")
